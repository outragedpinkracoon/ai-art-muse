...
class Critique
  ...
  def vision_generate(prompt, image_paths)
    Ollama.generate(Config::VISION_MODEL, prompt, images: image_paths.map { |p| Ollama.encode_image(p) }, options: {num_ctx: Config::OLLAMA_NUM_CTX})
  end
  ...
  def vision_chat(messages)
    ...
    Ollama.chat(Config::VISION_MODEL, formatted, options: {num_ctx: Config::OLLAMA_NUM_CTX})
  end
